...
public class ContactPersonEmergencyPhoneController {

	private final PhoneService service;
	private final VendorServiceClient vendorService;

	// CONTACT PERSON EMERGENCY
	@PutMapping(path = "/set-contact-person-emergency-number")
	...
	public ResponseEntity<Void> set(@RequestParam Long userId,
		...
		service.set(userId, PhonePurpose.CONTACT_PERSON_EMERGENCY, request);
		return ResponseEntity.accepted().build();
	}

	@PutMapping(path = "/verify-contact-person-emergency-number")
	@PreAuthorize("#userId == authentication.principal.userId")
	public ResponseEntity<Void> verify(@RequestParam Long userId){
		service.verify(userId, PhonePurpose.CONTACT_PERSON_EMERGENCY);
		return ResponseEntity.ok().build();
	}

	@GetMapping(path = "/retrieve-contact-person-emergency-number")
	@ResponseStatus(HttpStatus.OK)
	@PreAuthorize("#userId == authentication.principal.userId")
	public EntityModel<PhoneDto> retrieve(@RequestParam Long userId){
		return service.retrieve(userId, PhonePurpose.CONTACT_PERSON_EMERGENCY);
	}
