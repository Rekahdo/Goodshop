...
public class ContactPersonPhoneController {

	private final PhoneService service;
	private final VendorServiceClient vendorService;

	// CONTACT PERSON
	@PutMapping(path = "/set-contact-person-number")
	...
	public ResponseEntity<Void> set(@RequestParam Long userId,
		...
		service.set(userId, PhonePurpose.CONTACT_PERSON, request);
		return ResponseEntity.accepted().build();
	}

	@PutMapping(path = "/verify-contact-person-number")
	@PreAuthorize("#userId == authentication.principal.userId")
	public ResponseEntity<Void> verify(@RequestParam Long userId){
		service.verify(userId, PhonePurpose.CONTACT_PERSON);
		return ResponseEntity.ok().build();
	}

	@GetMapping(path = "/retrieve-contact-person-number")
	@ResponseStatus(HttpStatus.OK)
	@PreAuthorize("#userId == authentication.principal.userId")
	public EntityModel<PhoneDto> retrieve(@RequestParam Long userId){
		return service.retrieve(userId, PhonePurpose.CONTACT_PERSON);
	}
