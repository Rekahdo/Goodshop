...
public interface VendorServiceClient {
    ...
    @ResponseStatus(HttpStatus.OK)
    void businessPhoneAdded(@RequestParam Long userId,
                                   @RequestHeader(required = false) String apiKey);

    @PutMapping(path = "/set-business-phone-not-added")
    @ResponseStatus(HttpStatus.OK)
    void businessPhoneNotAdded(@RequestParam Long userId,
                                      @RequestHeader(required = false) String apiKey);

    @PutMapping(path = "/set-contact-phone-added")
    @ResponseStatus(HttpStatus.OK)
    void contactPhoneAdded(@RequestParam Long userId,
                                  @RequestHeader(required = false) String apiKey);

    @PutMapping(path = "/set-contact-phone-not-added")
    @ResponseStatus(HttpStatus.OK)
    void contactPhoneNotAdded(@RequestParam Long userId,
                                     @RequestHeader(required = false) String apiKey);

    @PutMapping(path = "/set-contact-emergency-phone-added")
    @ResponseStatus(HttpStatus.OK)
    void contactEmergencyPhoneAdded(@RequestParam Long userId,
                                           @RequestHeader(required = false) String apiKey);

    @PutMapping(path = "/set-contact-emergency-phone-not-added")
    @ResponseStatus(HttpStatus.OK)
    void contactEmergencyPhoneNotAdded(@RequestParam Long userId,
                                              @RequestHeader(required = false) String apiKey);
